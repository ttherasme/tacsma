...
def forest_growth_newA(matrix, value, tmatrix):
    """
    Append a Forest growth column and one new row.
    ...
    new_row = {col: 0 for col in df_A.columns}
    ...
    if tmatrix == 'A':
        new_row[flow_col] = 'Tree'
        new_row[flow_id_col] = 'FG_TREE'
        if unit_col:
            new_row[unit_col] = 'kg'
    elif tmatrix == 'B':
        new_row[flow_col] = 'Carbon dioxide'
        new_row[flow_id_col] = 'FG_CO2'
        if unit_col:
            new_row[unit_col] = 'kg'
    else:
        new_row[flow_col] = ''
