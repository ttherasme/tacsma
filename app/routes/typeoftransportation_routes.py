...
@typeoftransportation_bp.route('/listTotbystatus/<int:status>', methods=['GET'])
def listTotbystatus(status):
    if 'username' not in session:
        return jsonify([])
    ...
    if not query:
        mtransps = MTransp.query.filter(MTransp.State == status).order_by(MTransp.EntryDate.desc()).limit(20).all()
    else:
        like_query = f"%{query}%"
        filters = [
            MTransp.MTName.ilike(like_query),
