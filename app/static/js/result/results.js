...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function buildComparisonTable(data) {
        ...
        if (fullAnalysisResults && Array.isArray(fullAnalysisResults.individual_results)) {
            fullAnalysisResults.individual_results.forEach(result => {
                ...
                taskDetails[columnName] = {
                    ...
                    unit : result.entered_unit || "",
                    functionalUnit : result.entered_value || "",
        ...
        // Row 2: flow names
        // Row 3: functional unit + unit + flow names
        tableHtml += "<tr><th></th>";
        taskColumns.forEach(task => {
            const details = taskDetails[task] || {};
            
            const functionalUnit = details.functionalUnit || "";
            const unit = details.unit || "";
            const flowNames = details.flow_name || "";

            tableHtml += `<th>${functionalUnit} ${unit} ${flowNames}</th>`;
        });
        tableHtml += "</tr>";
